...
## 検証用の別リポジトリ（2026-07確定）

本リポジトリ（`https://live-ticket-data.pages.dev`）は**本番ユーザーが直接読んでいる配信**。
アプリ側のスキーマ変更や大きめのデータ収集作業を試したい場合は、完全に別のGitHubリポジトリ
`TsubasA-lu-c/live-ticket-data-dev`（Cloudflare Pages: `https://live-ticket-data-dev.pages.dev`）
で先に検証できる。過去に同一リポジトリの`dev`ブランチで運用しようとしたが、Xcodeのビルドキャッシュが
絡んで検証データが反映されない問題が解決せず、別リポジトリ方式に切り替えた経緯がある。

- 本リポジトリでの通常のデータ収集作業（アーティスト追加・更新等）は検証不要、直接mainに反映してよい
- 大きな仕様変更（スキーマ追加等）のみ、必要に応じて`live-ticket-data-dev`側で先に試す
- 両リポジトリ間の自動同期はない。検証OKなら本リポジトリ側で改めて同じ変更を行う
